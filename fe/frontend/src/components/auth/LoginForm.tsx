...
import {
  Eye, EyeOff, User, Lock, ChevronDown, Check, Coffee, Loader2,
} from 'lucide-react';
...
function RoleSelect({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <span
          className="flex items-center justify-center w-7 h-7 rounded-xl text-base shrink-0"
      ...
      <div
        ...
        className="absolute z-50 left-0 right-0 py-2 rounded-2xl border overflow-hidden"
        style={{
          top: 'calc(100% + 8px)',
          backgroundColor: '#FFFDF8',
          borderColor: '#E8D9C4',
          boxShadow:
            '0 20px 60px rgba(30,8,0,0.12), 0 8px 24px rgba(30,8,0,0.08)',
        ...
        {ROLES.map((r) => {
          ...
          return (
            ...
            >
              ...
              {active && (
                <Check size={15} strokeWidth={2.5} style={{ color: '#A0633A' }} />
              )}
            </button>
          );
        })}
...
function FloatingInput({
  ...
  leftIcon, rightSlot, autoComplete,
}: {
  ...
  return (
    <div>
      <label
        htmlFor={id}
        className="block text-xs font-semibold uppercase tracking-wider mb-2 transition-colors duration-200"
      ...
      >
        <span
          className="flex items-center justify-center pl-4 shrink-0 transition-colors duration-200"
        ...
        {rightSlot && (
          <span className="flex items-center pr-2 shrink-0">{rightSlot}</span>
        )}
      </div>
    </div>
  );
...
export default function LoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!name.trim() || !password) {
      setError('Nama pengguna dan password tidak boleh kosong.');
      return;
    }
    setLoading(true);
    setError('');
    ...
    try {
      ...
      let msg = 'Login gagal. Periksa kembali nama dan password Anda.';
      if (
        err &&
        typeof err === 'object' &&
        'response' in err &&
  ...
  return (
    <>
      ...
      >
        ...
        <div
          aria-hidden="true"
          className="pointer-events-none absolute -top-24 -left-28 w-96 h-96 rounded-full opacity-20"
          style={{ background: 'radial-gradient(circle, #D4850A 0%, transparent 70%)' }}
        />
        <div
          aria-hidden="true"
          className="pointer-events-none absolute -bottom-16 -right-20 w-72 h-72 rounded-full opacity-15"
          style={{ background: 'radial-gradient(circle, #A0522D 0%, transparent 70%)' }}
        />

        {/* ── floating card ── */}
        <main
          className={`ag-card relative z-10 w-full max-w-[440px] ${shake ? 'ag-shake' : ''}`}
        >
          <div
          ...
          >
            ...
            <div className="flex flex-col items-center text-center mb-8">
              <div
                className="coffee-ring flex items-center justify-center w-16 h-16 rounded-2xl mb-5 shrink-0"
                style={{
                  background: 'linear-gradient(145deg, #D4850A 0%, #A0522D 100%)',
                  boxShadow:
                    '0 8px 20px rgba(164,82,10,0.35), inset 0 1px 0 rgba(255,255,255,0.25)',
                }}
              >
              ...
              <h1
                className="text-2xl sm:text-3xl font-bold tracking-tight leading-tight mb-1.5"
                style={{ color: '#1A0800', fontFamily: 'inherit' }}
              >
            ...
            <form onSubmit={handleSubmit} className="space-y-5" noValidate>
              <FloatingInput
                ...
                leftIcon={<User size={17} strokeWidth={2} />}
              />
              ...
              {error && (
                <div
                  ...
                  role="alert"
                  style={{
                    backgroundColor: 'rgba(180,30,30,0.07)',
                    color: '#9B2226',
              ...
              <button
                ...
                style={{
                  background: loading
                    ? '#A0633A'
                    : 'linear-gradient(135deg, #C07A24 0%, #8B3A10 100%)',
                  color: '#FFFBF4',
                  boxShadow: loading
                    ? 'none'
                    : '0 4px 12px rgba(160,82,10,0.35), 0 2px 4px rgba(60,20,0,0.20)',
                }}
                onMouseEnter={(e) => {
                  if (loading) return;
                  e.currentTarget.style.transform = 'translateY(-2px)';
                  e.currentTarget.style.boxShadow =
                    '0 8px 20px rgba(160,82,10,0.45), 0 4px 8px rgba(60,20,0,0.25)';
                }}
                onMouseLeave={(e) => {
        ...
        {/* footer */}
        <footer
          className="mt-8 text-center text-xs select-none"
          style={{ color: 'rgba(120,80,40,0.50)' }}
        >
